...
namespace NoCarsAllowed
{
    public partial class Form1 : Form
    {
        ...
        public static bool ConsoleInv = true;   // Переменная для нанесения на pictureBox активной формы q3_console.png при переключении форм
        ...
        {   // Используем оператор итераций while для определения нового положения изображения и его нанесения пока первая форма активна
            ...
            while (!ConsoleInv)
            {
                Frm2.pictureBox1.Invalidate();   // Вызов события для нанесения изображения на первый компонент pictureBox1 расположенный снизу второй формы
                Frm2.pictureBox2.Invalidate();   // Вызов события для нанесения изображения на второй компонент pictureBox2 расположенный сверху второй формы
                y += 1;
                x += 1;
                if (x > 785)
                    x = 0;
        ...
        private void pictureBox1_On_Paint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.DrawImage(image, new Point(x - 784, y - 360));
        }
        ...
        // Метод для имитации нажатия кнопки, name - Название компонента, ioo - Тип преобразования
        public static async void Pressing(Label name, char ioo)
        {   // Т.к. курсор находится над компонентом метки при нажатии, присваиваем fontChange - истину, для избежания проблем с необратимым преобразованием текста
            fontChange = true;
            LastName = name; // Присваиваем имя нажатой кнопке
            // Инструкции для увеличения шрифта
            if (ioo == '+')
            {
                fontSize = name.Font.Size;  // Сохраняем базовый размер шрифта для метода UnBlinking перед преобразованием
                for (int i = 0; i <= 2; i++) // Оператор for для увеличения шрифта на 2 размера
                {   // Присваивание нового шрифта тексту на основании старого уменьшив его размер
                    name.Font = new Font(name.Font.Name, name.Font.Size - 1);
                    await Task.Delay(5);    // Удерживание изменения размера шрифта
                }
            }
            // Инструкции для уменьшения шрифта
            if (ioo == '-')
            {   
                name.Font = new Font(name.Font.Name, fontSize); // Возвращаем исходный размер шрифта 
            }
        }
        // Метод для изменения содержимого текста, Name - Название компонента, t - Текст для присваивания
        public static void LanguageChange(Label Name, string t)
        {
            Name.Text = t;
        }
        ...
        private void LANGUAGE_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            {   // Считывание переменной языка из list.tхt
                ...
                Pressing(LANGUAGE, '-');    // Имитация нажатия
            }
        }
        ...
        private async void START_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {   
                Frm2.Location = this.Location;  // Установка позиции второй формы на текущую
                Frm2.GetLanguage(); // Установка языка второй формы
                Frm2.Show();
                ConsoleInv = false; // Прекращение движения изображения данной формы и возобновление во второй
